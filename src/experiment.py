...
import numpy as np
import pandas as pd
from .dataset import get_values_and_labels_index, split_and_scale, windowing, get_feature_names
...
class ExperimentInstance:
    ...
    def calculate_metrics(self, model, history, data_test, duration):
        metrics = pd.DataFrame(model.evaluate(data_test, return_dict=True), index=[0])
